...
export default function SwaggerBrowser({ open, onSelect, onClose, onApiBase, apiBaseOverride }: Props) {
  const { urlRaiz } = useSessionStore();
  // La base es SIEMPRE la raíz de la empresa activa (entre empresas solo cambia el host).
  // El override por módulo se envía aparte; el backend toma su ruta y la compone sobre esta raíz.
  const urlBase = urlRaiz ? urlRaiz.replace(/\/v3$/i, '').replace(/\/+$/, '') : '';
  const override = apiBaseOverride && apiBaseOverride.trim() ? apiBaseOverride.trim() : '';
  ...
  useEffect(() => {
    ...
    const overrideParam = override ? `&override=${encodeURIComponent(override)}` : '';
    api.get(`/swagger-proxy?urlBase=${encodeURIComponent(urlBase)}${overrideParam}`)
      .then(({ data }) => {
        setSpec(data);

        const swaggerUrl = typeof data._swaggerUrl === 'string' ? data._swaggerUrl : '';
        if (swaggerUrl && onApiBase) {
          // La base de endpoints es lo que va antes de /swagger/… (cubre /docs/v1 y otras variantes).
          onApiBase(swaggerUrl.replace(/\/swagger\/.*$/i, ''));
        }
      })
  ...
  }, [onApiBase, open, urlBase, override]);
