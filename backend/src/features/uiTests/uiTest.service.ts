...
export interface UiTest {
    ...
    baseline?: string;      // base64 PNG de referencia
    hasBaseline?: boolean;
}
...
export interface UiTestResult {
    ...
    mismatchPct?: number;   // % de píxeles distintos vs baseline
    diff?: string;          // base64 PNG del diff resaltado
...
async function tryWebLogin(page: Page): Promise<void> {
    ...
    if (previousAttempt?.url === currentUrl && (Date.now() - previousAttempt.attemptedAt) < 10000) {
        console.log('[UI Playwright] Skipping duplicate login attempt', { currentUrl, count: previousAttempt.count });
        return;
    }
    ...
    // NO se usan credenciales de desarrollo: el ingreso es SIEMPRE por keyC.
    const loginAspx = currentUrl.replace(/Login_iv\.aspx.*$/i, 'Login.aspx');

    let key = '';
    try {
        key = await torreService.getSsoKey();
    } catch (error) {
        console.log('[UI Playwright] No se pudo obtener la keyC de Torre:', error instanceof Error ? error.message : String(error));
        return;
    }

    const fields = torreService.loginFields(key);
    console.log('[UI Playwright] Ingreso keyC (Torre) ->', loginAspx, '| usuario:', fields.usuario);
    ...
    try {
        ...
        if (/Login_iv\.aspx/i.test(page.url())) {
            console.log('[UI Playwright] keyC NO autenticó (sigue en Login_iv). Revisar keyC / usuario.', { url: page.url() });
        } else {
            console.log('[UI Playwright] Ingreso keyC OK', { newUrl: page.url() });
        }
    } catch (error) {
        console.log('[UI Playwright] Error en ingreso keyC:', error instanceof Error ? error.message : String(error));
    }
}
...
const VISUAL_THRESHOLD_PCT = 0.5;

function compareScreens(baselineB64: string, currentB64: string): { mismatchPct: number; diff: string } {
    const a = PNG.sync.read(Buffer.from(baselineB64, 'base64'));
    const b = PNG.sync.read(Buffer.from(currentB64, 'base64'));
    const width = Math.min(a.width, b.width);
    const height = Math.min(a.height, b.height);
    // recorta ambas a la región común para poder comparar aunque cambie el tamaño
    const crop = (src: any) => { const out = new PNG({ width, height }); for (let y=0;y<height;y++){ for(let x=0;x<width;x++){ const si=(src.width*y+x)<<2; const di=(width*y+x)<<2; out.data[di]=src.data[si]; out.data[di+1]=src.data[si+1]; out.data[di+2]=src.data[si+2]; out.data[di+3]=src.data[si+3]; } } return out; };
    const ca = crop(a), cb = crop(b);
    const diff = new PNG({ width, height });
    const changed = pixelmatch(ca.data, cb.data, diff.data, width, height, { threshold: 0.1 });
    const totalDims = a.width !== b.width || a.height !== b.height;
    const base = width * height || 1;
    let pct = (changed / base) * 100;
    if (totalDims) pct = Math.max(pct, 100 * (1 - (width*height)/Math.max(a.width*a.height, b.width*b.height))); // penaliza cambio de tamaño
    return { mismatchPct: Math.round(pct * 100) / 100, diff: PNG.sync.write(diff).toString('base64') };
}
...
export class UiTestService {
    list(mod: string, sub?: string, page?: string) {
        // No enviar el baseline (base64) en el listado para no inflar el payload;
        // se expone un booleano hasBaseline en su lugar. lastResult se mantiene intacto.
        return normalizeTests(read(testsPath(mod, sub, page)))
            .map((t) => ({ ...t, baseline: undefined, hasBaseline: !!t.baseline }));
    }
    ...
    async run(mod: string, id: string, sub?: string, page?: string, adproToken?: any, overrideUrl?: string, sessionContext?: AdproSessionContext): Promise<UiTestResult> {
        ...
        // Regresión visual: comparar el screenshot actual contra el baseline de referencia.
        // Es una señal independiente de result.ok (un diff visual no falla el test funcional).
        if (!result.screenshot) {
            result.visual = 'na';
        } else if (!test.baseline) {
            test.baseline = result.screenshot;
            result.visual = 'baseline';
        } else {
            try {
                const cmp = compareScreens(test.baseline, result.screenshot);
                result.mismatchPct = cmp.mismatchPct;
                result.diff = cmp.diff;
                result.visual = cmp.mismatchPct <= VISUAL_THRESHOLD_PCT ? 'ok' : 'diff';
            } catch (e) {
                result.visual = 'na';
            }
        }
        ...
        // Historial compartido: registrar la corrida UI para que la analítica la incluya.
        void recordRun({
            id: randomUUID(),
            flowId: id,
            module: mod,
            ...
            assertionTotal: 0,
            assertionFailed: 0,
            empresaNombre: sessionContext?.empresaNombre,
            sucursalNombre: sessionContext?.sucursalNombre,
    ...
    async setBaseline(mod: string, id: string, sub?: string, page?: string): Promise<UiTest> {
        const file = testsPath(mod, sub, page);
        const tests = normalizeTests(read(file));
        const test = tests.find(t => t.id === id);
        if (!test) throw new Error(`Test ${id} no encontrado`);

        const screenshot = test.lastResult?.screenshot ?? test.baseline;
        if (!screenshot) throw new Error(`Test ${id} no tiene captura para promover como baseline`);

        test.baseline = screenshot;
        // Limpiar el diff obsoleto del último resultado tras aprobar el nuevo baseline.
        if (test.lastResult) {
            test.lastResult.diff = undefined;
            test.lastResult.mismatchPct = undefined;
...
export const uiTestService = new UiTestService();
