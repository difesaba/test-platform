...
import { UiTestService } from './uiTest.service';
import { resolveRuntimeUrl } from '../../shared/helpers/runtime-url';
...
export class UiTestsController {
    ...
    componentPickerStatus = async (req: Request, res: Response) => {
        try {
            res.json(await svc.getComponentPickerStatus((req.params.id as string)));
        } catch (e: any) {
            res.status(500).json({ error: e.message });
        }
    };
    ...
    cancelComponentPicker = async (req: Request, res: Response) => {
        try {
            await svc.cancelComponentPicker((req.params.id as string));
            res.json({ ok: true });
        } catch (e: any) {
            res.status(500).json({ error: e.message });
        }
    ...
    run = async (req: Request, res: Response) => {
        ...
        try {
            ...
            const result = await svc.run(mod, (req.params.id as string), submodule, page, adproToken, overrideUrl, getSessionContext(req));
            res.json(result);
        } catch (e: any) {
            res.status(500).json({ error: e.message });
        }
    ...
    setBaseline = async (req: Request, res: Response) => {
        ...
        if (!mod) return res.status(400).json({ error: 'Falta module' });
        try {
            const result = await svc.setBaseline(mod, (req.params.id as string), submodule, page);
            res.json(result);
        } catch (e: any) {
            res.status(500).json({ error: e.message });
        }
